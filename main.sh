...
# &> /dev/null means:“Send both standard output and error messages to /dev/null — i.e., silently suppress all output.”
if ! command -v dialog &> /dev/null; then
#If the dialog command is not installed on the system, then silently continue and run the code inside the if-block.
    echo "dialog is not installed. Installing..."
    sudo apt-get update && sudo apt-get install -y dialog
    if [ $? -ne 0 ]; then
# $? This is a special shell variable that stores the exit status of the last executed command.
        echo "Failed to install dialog. Please install it manually."
        exit 1
    fi
fi
# The dollar sign ($) is used in shell scripting to reference the value of a variable.
...
#Flags explained
# grep -v  → “show everything versus (except) the match”
# \|     → acts like OR inside the pattern
# wc -l → word-count, but -l means “how many lines”
# cut -d: -f1 → cut the string using : as delimiter and get the first field
get_total_users() {
...
# who  → command to show who is logged in
# wc -l  → word-count, but -l means “how many lines”
# 2>/dev/null  → redirect standard error to /dev/null
#  -n "$logged_in" ->	Is the variable non-empty (someone is logged in)?
...
# -h	Hides the header of w output
# 2>/dev/null	Suppresses errors (like if w doesn't work)
# -n "$session_info"	Checks if output is not empty (someone is logged in)
# \n and \t	Used for new lines and tabs in text formatting
# show_text	Custom function to show output in a dialog box
